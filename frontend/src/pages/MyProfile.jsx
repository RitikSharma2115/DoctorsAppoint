...
const MyProfile = () => {
  ...
  return userData && (
    <div className='flex flex-col max-w-lg gap-2 text-sm'>
      <p className='font-medium text-3xl text-neutral-900 mb-4'>My Profile:</p>
      ...
      <div>
        ...
        <div className='grid grid-cols-[1fr_3fr] gap-y-2.5 mt-3 text-neutral-700'>
          ...
          {
            isEdit
            ? <select className='bg-gray-200 max-w-20' onChange={(e) => setUserData(prev => ({...prev, gender: e.target.value}))} value={userData.gender}>
          ...
          {
            isEdit
            ? <input className='bg-gray-200 max-w-32' type="date" onChange={(e) => setUserData(prev => ({...prev, dob: e.target.value}))} value={userData.dob}/>
      ...
      <div className='mt-10'>
        {
          isEdit
          ? <button className='border border-primary px-8 py-2 rounded-full hover:bg-primary hover:text-white hover:scale-105 transition-all'  onClick={updateUserProfileData}>Save information</button>
          : <button className='border border-primary px-8 py-2 rounded-full hover:bg-primary hover:text-white hover:scale-105 transition-all'  onClick={()=>setIsEdit(true)}>Edit</button>
        }
      </div>
